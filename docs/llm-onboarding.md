...
Calendar `checkIn` **does not** insert `checkins` and **does not** set `beds.occupied`. Guest ID book is Self Check-in / Records. Aiosell `pah: false` is **prepaid** (do not collect; do **not** set `amountPaid = amountTotal` — that cash never hit Goko). Detail Paid/Balance for prepaid is `displayedStayPayment` (shows total / ₹0). `pah: true` is `pay_at_hotel`. Omitted `pah` is `unknown`, not prepaid. Booking detail **Collect payment** / **Payment done** is `collectionCopy(paymentStatus, balance)`. Desk **Collected** must also set `paymentStatus: paid` or the label stays Collect payment. Walk-in `createBooking` stays `unknown`; red Balance still means collect.
...
| `canCheckIn` / `canCheckOut` | **not in Users UI** | Calendar buttons also accept grantable `canAddBooking`. API `checkIn`/`checkOut` is OR of dedicated key + `canAddBooking`. |
