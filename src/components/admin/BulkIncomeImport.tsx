...
export function BulkIncomeImport({
  password,
  username,
  role,
}: {
  ...
  if (role !== "admin") {
    return (
      <p className="py-10 text-center text-brand-green-dark/50">
        Only admins can bulk upload income.
      </p>
    );
  }
  ...
  const handleDownloadTemplate = async () => {
    setDownloading(true);
    try {
      const payload: Record<string, any> = { password, action: "incomeTemplate" };
      if (username) payload.username = username;
      const res = await fetch("/api/admin/bulk-import-accounts", {
        method: "POST",
      ...
      a.href = url;
      a.download = "income_import_template.xlsx";
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      formData.append("importType", "income");
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-brand-green-dark/60">
              Download the Excel template with pre-defined columns. Fill in your
              income data, then upload it below.
            </p>
            <div className="mt-3">
              <Button
                type="button"
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-brand-green-dark/60">
              ...
              duplicates, and insert new income records.
            </p>
          </div>
        </div>
        ...
        >
          {file ? (
            ...
            <>
              ...
              <label
                htmlFor="bulk-income-import-file"
      ...
      <div className="rounded-xl bg-brand-sand/50 p-4 text-xs text-brand-green-dark/60">
        ...
        <ul className="mt-1 list-inside list-disc space-y-0.5">
          <li>Download the template, fill your income data under the column headings</li>
          <li>Required fields: date, amount</li>
          <li>Source: stay (default), food, or other</li>
          <li>Type: cash (default) or online</li>
          <li>Account names must match existing accounts in Account Settings</li>
          <li>Amounts should be in rupees (e.g. 1000 for ₹1000)</li>
          <li>
            <strong>Safe retries:</strong> Re-uploading the same file skips
            already-imported rows (matched by date + amount + source + description)
          </li>
        </ul>
      </div>
    </div>
